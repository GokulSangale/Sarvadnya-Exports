    ...
    <header
      className={cn(
        ...
        scrolled
          ...
          : "bg-ink/40 backdrop-blur-sm py-6"
      )}
    >
      <div className="mx-auto max-w-7xl px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          <span
            className={cn(
              ...
              scrolled ? "text-pomegranate" : "text-cream drop-shadow-sm"
            )}
          >
            Sarvadnya
          </span>
          <span
            className={cn(
              "font-mono text-[11px] tracking-[0.2em] uppercase hidden sm:inline transition-colors",
              scrolled ? "text-forest/80" : "text-cream/85"
            )}
          >
            Exports
          </span>
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                ...
                scrolled ? "text-ink/80 hover:text-pomegranate" : "text-cream hover:text-gold"
              )}
            >
              {link.label}
            </Link>
        ...
        <button
          className={cn("md:hidden transition-colors", scrolled ? "text-ink" : "text-cream")}
